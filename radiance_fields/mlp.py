...
class SinusoidalEncoder(nn.Module):
    ...
    def __init__(self, x_dim, min_deg, max_deg, use_identity: bool = True):
        ...
        self.freq_mask = torch.ones(2*x_dim*L + (x_dim if self.use_identity else 0)).view(1, -1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # apply frequency regularization mask
        latent *= self.freq_mask.to(x.device).tile((x.shape[0], 1))
        return latent
